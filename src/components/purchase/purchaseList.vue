...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/purchaseDetail?orderId=' + this.Gbl.GainUrlValueFun('orderId') + '&orderType=' + this.Gbl.GainUrlValueFun('orderType'),
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: false,
      ...
      this.axios({
        ...
        if ((response.status + '') === '200') {
          if ((response.data.status + '') === '200') {
            ...
            this.router.push('/successResult?downUrl=purchaseSearch&downName=' + encodeURIComponent(downName))
          } else if ((response.data.errorCode + '') === '4000' || (response.data.errorCode + '') === '4001' || (response.data.errorCode + '') === '4002') {
            this.$emit('fInfoFun', ['errorInfo', response.data.msg, 'errorCode'])
          } else {
            console.log(response.data)
    ...
    totalListData () {
      ...
      for (var j = 0; j < newSurplusList.length; j++) {
        this.surplusList.push({
          ...
          quantity_scaned: this.Gbl.LangFun(this.WL.text113, this.Gbl.wayMapUserLang) + '：' + ((newSurplusList[j].quantity_received - 0) + (newSurplusList[j].quantity_scaned - 0)),
          result: this.Gbl.LangFun(this.WL.text114, this.Gbl.wayMapUserLang) + '：' + ((newSurplusList[j].quantity - 0) - ((newSurplusList[j].quantity_received - 0) + (newSurplusList[j].quantity_scaned - 0)))
        })
      }
