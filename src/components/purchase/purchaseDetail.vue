  ...
  <div class='pageVM' id='pageVM' >
    ...
    <div class="content">
      ...
      <div class="item2">
        ...
        <div class="right">
          <input type="text" class="barCode" v-model="barCode"
          v-on:click.stop="barCodeClickFun" v-on:keyup.enter="barCodeEnter"
          ref="barcode"/>
          <div class="barCodeList" v-show="barCodeListDir">
            ...
            v-on:click="barCodeItemClick($event)">{{item.itemName}}</div>
          </div>
        </div>
      </div>
      <div class="item9" v-if="false">
...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/purchaseSearch',
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: true,
        title: this.Gbl.LangFun(this.WL.text3, this.Gbl.wayMapUserLang),
        titleDir: true
      },
      alreadyNum: 0,
      barCode: '',
  ...
  methods: {
    ...
    exitScanClick () {
      this.router.push('/purchaseList?orderId=' + this.orderCode + '&orderType=' + this.orderType)
    },
