      ...
      <div class="item2">
        ...
        <div class="right">
          ...
          <div class="barCodeList" v-show="barCodeListDir">
            ...
            v-bind:key="'index'+index">{{item.itemName}}</div>
          </div>
        </div>
      </div>
      <div class="item9">
...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/purchaseReturnSearch',
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: true,
  ...
  methods: {
    ...
    barCodeClickFun () {
      ...
      var surplusList = []
      for (var i = 0; i < this.orderListLen; i++) {
        if ((this.orderList[i].result - 0) === 0 || (this.orderList[i].result + '') === (this.orderList[i].quantityScaned + '')) {
    ...
    exitScanClick () {
      this.router.push('/purchaseReturnList?orderId=' + this.orderCode + '&orderType=' + this.orderType)
    },
    ...
    gainOrderDetail () {
      ...
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTransationDetailList' + this.Gbl.commonVariaList + '&tranid=' + this.orderCode + '&transactionType=PO_RETURN'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
    ...
    alreadyNum (val) {
      if ((this.pdtDetail.waitNum - 0) <= 0 && (this.pdtDetail.pdtNum - 0) > 0) {
        ...
        if ((this.alreadyNum - 0) > (this.pdtDetail.waitNum - 0)) {
          this.$emit('fInfoFun', ['errorInfo', this.Gbl.LangFun(this.WL.text106, this.Gbl.wayMapUserLang), 'alreadyNum'])
          this.alreadyNum = this.pdtDetail.waitNum
