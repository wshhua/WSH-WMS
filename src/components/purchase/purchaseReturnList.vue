  ...
  <div class='pageVM' id='pageVM' >
    ...
    <div class="content">
      ...
      <div class="selectedOrder">
        <div class="alreadyOrder" ref="already-button" v-bind:class="{active:alreadyDisplay}" v-on:click="alreadyClick">{{this.Gbl.LangFun(this.WL.text32, this.Gbl.wayMapUserLang)}}</div>
        <div class="surplusOrder" ref="surplus-button" v-bind:class="{active:!alreadyDisplay}" v-on:click="surplusClick">{{this.Gbl.LangFun(this.WL.text33, this.Gbl.wayMapUserLang)}}</div>
      </div>
      <ul class="orderList">
        <li class="item" v-for="(site,index) in orderList" v-bind:key="'index'+index">
          <div class="itemInfo">
            ...
            <div class="itemNumInfo">
              ...
              <div class="itemNum">{{listText.text3}}:{{site.quantityScaned}}</div>
              <div class="itemOthers">{{listText.text4}}:{{(site.result - 0) - (site.quantityScaned)}}</div>
            </div>
          </div>
...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/purchaseReturnDetail?orderId=' + this.Gbl.GainUrlValueFun('orderId') + '&orderType=' + this.Gbl.GainUrlValueFun('orderType'),
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: false,
        title: this.Gbl.LangFun(this.WL.text4, this.Gbl.wayMapUserLang),
        titleDir: true
      },
      listText: {
        ...
        text3: this.Gbl.LangFun(this.WL.text71, this.Gbl.wayMapUserLang),
        text4: this.Gbl.LangFun(this.WL.text72, this.Gbl.wayMapUserLang)
      },
      alreadyDisplay: true,
      alreadyList: [],
      goPageIndex: 1,
  ...
  methods: {
    ...
    submintFun () {
      ...
      this.$emit('loadingFun', true)
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/submitTransaction'
      ...
      this.axios({
        ...
        if ((response.status + '') === '200') {
          if ((response.data.status + '') === '200') {
            ...
            this.router.push('/successResult?downUrl=purchaseReturnSearch&downName=' + encodeURIComponent(downName))
          } else if ((response.data.errorCode + '') === '4000' || (response.data.errorCode + '') === '4001' || (response.data.errorCode + '') === '4002') {
            this.$emit('fInfoFun', ['errorInfo', response.data.msg, 'errorCode'])
          } else {
            console.log(response.data)
  ...
  watch: {
    ...
    totalListData () {
      var alreadyData = []
      var surplusData = []
      var totalListLen = this.totalListData.length
      for (var i = 0; i < totalListLen; i++) {
        if ((this.totalListData[i].result - 0) === 0 || (this.totalListData[i].result - 0) === (this.totalListData[i].quantityScaned - 0)) {
          alreadyData.push(this.totalListData[i])
        } else {
          surplusData.push(this.totalListData[i])
        }
      }
      this.alreadyList = alreadyData
      this.surplusList = surplusData
