...
export default {
  ...
  methods: {
    ...
    gainAllotList () {
      ...
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=PO_RETURN'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
    ...
    searchClick () {
      ...
      if (this.purchaseNum === '') {
        ...
        this.router.push('/purchaseReturnDetail?orderId=' + this.purchaseNum + '&orderType=' + this.orderType)
      } else if (!this.isCanSubmit) {
