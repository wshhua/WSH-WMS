...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/checkDetail?orderId=' + this.Gbl.GainUrlValueFun('orderId') + '&orderType=' + this.Gbl.GainUrlValueFun('orderType'),
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: false,
        title: this.Gbl.LangFun(this.WL.text7, this.Gbl.wayMapUserLang),
        titleDir: true
      },
      listText: {
        ...
        text3: this.Gbl.LangFun(this.WL.text70, this.Gbl.wayMapUserLang)
      },
      alreadyDisplay: true,
      alreadyList: [],
      goPageIndex: 1,
  ...
  methods: {
    ...
    gainOrderDetail () {
      ...
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTransationDetailList' + this.Gbl.commonVariaList + '&tranid=' + this.orderCode + '&transactionType=INVENTORY_COUNT'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
      ...
      this.axios({
        ...
        if ((response.status + '') === '200') {
          if ((response.data.status + '') === '200') {
            ...
            this.router.push('/successResult?downUrl=checkSearch&downName=' + encodeURIComponent(downName))
          } else if ((response.data.errorCode + '') === '4000' || (response.data.errorCode + '') === '4001' || (response.data.errorCode + '') === '4002') {
            this.$emit('fInfoFun', ['errorInfo', response.data.msg, 'errorCode'])
          } else {
            console.log(response.data)
  ...
  watch: {
    ...
    totalListData () {
      ...
      for (var i = 0; i < totalListLen; i++) {
        if ((this.totalListData[i].quantityScaned - 0) > 0) {
