...
export default {
  ...
  methods: {
    ...
    gainAllotList () {
      ...
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=INVENTORY_COUNT'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
      ...
      for (var i = 0; i < this.orderListLen; i++) {
        if (this.purchaseNum.toLowerCase() === this.orderList[i].tranid.toLowerCase()) {
          if (this.orderList[i].lockedByUser === '0' || this.orderList[i].lockedByUser === this.Gbl.wayMapUserName) {
            ...
            this.orderType = 'INVENTORY_COUNT'
    ...
    searchClick () {
      ...
      if (this.purchaseNum === '') {
        ...
        this.router.push('/checkDetail?orderId=' + this.purchaseNum + '&orderType=' + this.orderType)
      } else if (!this.isCanSubmit) {
