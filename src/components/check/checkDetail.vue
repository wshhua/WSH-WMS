      ...
      <div class="item2">
        ...
        <div class="right">
          ...
          <div class="barCodeList" v-show="barCodeListDir">
            <div class="barCodeItem" v-on:click="barCodeItemClick($event)" v-bind:class="{active:(item.result===0 ||item.result==item.quantityScaned)}"
...
export default {
  ...
  methods: {
    ...
    barCodeClickFun () {
      ...
      this.barCodeList = this.orderList
    },
    clearLoadFun () {
    ...
    exitScanClick () {
      this.router.push('/checkList?orderId=' + this.orderCode + '&orderType=' + this.orderType)
    },
    ...
    progressFun () {
      ...
      for (var i = 0; i < this.totalSku; i++) {
        if ((this.orderList[i].quantityScaned - 0) > 0) {
          this.progress++
        }
      }
    },
    ...
    submintClick () {
      if (this.inTask) {
        if ((this.alreadyNum + '') === (this.pdtDetail.waitNum + '')) {
          ...
          this.$emit('exitEvent', [this.Gbl.LangFun(this.WL.text120, this.Gbl.wayMapUserLang), 'submint'])
        }
      } else {
        this.clearLoadFun()
