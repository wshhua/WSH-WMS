        ...
        <li class="item" v-for="(site,index) in orderList" v-bind:key="'index'+index">
          <div class="itemInfo">
            ...
            <div class="itemNumInfo">
              ...
              <div class="itemOthers">{{listText.text4}}:{{site.result}}</div>
            </div>
          </div>
...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/allotDetailIn?orderId=' + this.Gbl.GainUrlValueFun('orderId') + '&orderType=' + this.Gbl.GainUrlValueFun('orderType'),
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: false,
      ...
      listText: {
        ...
        text3: this.Gbl.LangFun(this.WL.text64, this.Gbl.wayMapUserLang),
        text4: this.Gbl.LangFun(this.WL.text65, this.Gbl.wayMapUserLang)
      },
      alreadyDisplay: true,
      alreadyList: [],
      goPageIndex: 1,
  ...
  methods: {
    ...
    submintFun () {
      ...
      this.$emit('loadingFun', true)
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/submitTransaction?entry=IN'
      ...
      this.axios({
        ...
        if ((response.status + '') === '200') {
          if ((response.data.status + '') === '200') {
            ...
            this.router.push('/successResult?downUrl=allotSearchIn&downName=' + encodeURIComponent(downName))
          } else if ((response.data.errorCode + '') === '4000' || (response.data.errorCode + '') === '4001' || (response.data.errorCode + '') === '4002') {
            this.$emit('fInfoFun', ['errorInfo', response.data.msg, 'errorCode'])
          } else {
            console.log(response.data)
