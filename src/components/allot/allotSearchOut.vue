...
export default {
  ...
  methods: {
    ...
    gainAllotList () {
      ...
      // 获取调拨单清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=TO&entry=OUT'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
      ...
      // 获取调拨单出库清单数据
      var loginUrl2 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=TO_WAVE&entry=OUT'
      this.axios({
        method: 'get',
        url: loginUrl2,
        data: '',
      ...
      if (this.purchaseNum === '') {
        ...
        if ((this.orderList[this.selectedIndex].locationId + '') === locationAddr) {
          if (this.orderList[this.selectedIndex].lockedByUser === '0' || this.orderList[this.selectedIndex].lockedByUser === this.Gbl.wayMapUserName) {
            this.router.push('/allotDetailOut?orderId=' + this.purchaseNum + '&orderType=' + this.orderType)
          } else {
