  ...
  <div class='pageVM' id='pageVM' >
    ...
    <div class="content">
      ...
      <div class="item2">
        ...
        <div class="right">
          <input type="text" class="barCode" v-model="barCode" v-on:click.stop="barCodeClickFun" v-on:keyup.enter="barCodeEnter" ref='inputbarcode'/>
          <div class="barCodeList" v-show="barCodeListDir">
            <div class="barCodeItem" v-on:click="barCodeItemClick($event)"
            v-bind:class="{active:(item.result===0 ||item.result==item.quantityScaned)}"
            v-for="(item,index) in barCodeList" v-bind:key="'index'+index">{{item.itemName}}</div>
          </div>
        </div>
      </div>
      <div class="item5" v-show="binNameDivDir">
        ...
        <div class="right">
          <input type="text" class="binName" v-model="binName" v-on:keyup.enter="binNameEnter"
          v-on:click.stop="binNameClickFun" ref='inputbinname'/>
          <div class="binNameList" v-show="binNameListDir">
            <div class="binNameItem" v-for="(item,index) in binNameData"
            v-on:click="itemClick('binName', $event)" v-bind:key="'index'+index">{{item.binName}}</div>
          </div>
        </div>
      </div>
      <div class="item9" v-show="batchNumDivDir">
        ...
        <div class="right">
          <input type="text" class="batchNum" v-model="batchNum" v-on:keyup.enter="batchNumEnter"
          v-on:click.stop="batchNumClickFun" ref='inputbatchnum'/>
          <div class="batchNumList" v-show="batchNumListDir">
            <div class="batchNumItem" v-for="(item,index) in batchNumData"
            v-on:click="itemClick('batchNum', $event)" v-bind:key="'index'+index">{{item.lotNumber}}</div>
          </div>
        </div>
      </div>
      <div class="item3">
        <div class="left">{{this.Gbl.LangFun(this.WL.text20, this.Gbl.wayMapUserLang)}}:</div>
        <div class="right"><p>{{pdtDetail.pdtName}}</p></div>
      </div>
      <div class="item4">
      ...
      <div class="item6">
        ...
        <div class="right"><span>{{pdtDetail.pdtNum}}(</span><span>{{alreadyNum}}</span><span>/</span><span>{{pdtDetail.waitNum}}</span><span>)</span><span>当前库存：{{binNameMaxNum}}</span></div>
      </div>
      <div class="item10">
...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/salesSearch',
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: true,
        title: this.Gbl.LangFun(this.WL.text1, this.Gbl.wayMapUserLang),
        titleDir: true
      },
      alreadyNum: 0,
      alreadySubmintData: [],
  ...
  methods: {
    ...
    exitScanClick () {
      this.router.push('/salesList?orderId=' + this.orderCode + '&orderType=' + this.orderType)
    },
    gainBinListData (val1, val2) {
      ...
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTransationBinName' + this.Gbl.commonVariaList + '&itemId=' + val1
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
    ...
    gainBinSubmintData (val1) {
      ...
      this.$emit('loadingFun', true)
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTransactionItemInventoryDetails' + this.Gbl.commonVariaList + '&itemId=' + val1 + '&trainid=' + this.orderCode + '&entry=OUT&binNumber='
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
    ...
    submintFun () {
      ...
      this.$emit('loadingFun', true)
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/saveTransationDetailLine'
      ...
      this.axios({
        ...
        if ((response.status + '') === '200') {
          if ((response.data.status + '') === '200') {
            ...
            console.log(response.data)
            this.$emit('fInfoFun', ['errorInfo', this.Gbl.LangFun(this.WL.text100, this.Gbl.wayMapUserLang), 'submint'])
          }
        } else {
          this.$emit('fInfoFun', ['errorInfo', response.msg])
        }
    ...
    binName (val1) {
      if (this.itemType === 'I' || this.itemType === 'L') {
        ...
        if (this.alreadySubmintNumBackup !== '' && (this.alreadyNum - 0) !== (this.alreadySubmintNumBackup.split('/')[0] - 0)) {
          ...
          this.binNameMaxNum = 0
          this.batchNum = ''
          var dataLenB = this.binNameData.length
          for (var j = 0; j < dataLenB; j++) {
