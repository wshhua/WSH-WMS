...
export default {
  ...
  methods: {
    ...
    gainAllotList () {
      ...
      // 获取调拨单清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=SO_WAVE'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
      ...
      // 获取调拨单出库清单数据
      var loginUrl2 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=SO'
      this.axios({
        method: 'get',
        url: loginUrl2,
        data: '',
    ...
    isCanSubmitFun () {
      for (var i = 0; i < this.orderListLen; i++) {
        if (this.purchaseNum.toLowerCase() === this.orderList[i].tranid.toLowerCase()) {
          if (this.orderList[i].lockedByUser === '0' || this.orderList[i].lockedByUser === this.Gbl.wayMapUserName) {
            ...
            this.lockedByName = this.orderList[i].lockedByUser
          }
          break
          ...
          for (var j = 0; j < this.orderListLen; j++) {
            if (this.purchaseNum.toLowerCase() === (this.orderList[j].trackingNumber + '').toLowerCase()) {
              if (this.orderList[j].lockedByUser === '0' || this.orderList[j].lockedByUser === this.Gbl.wayMapUserName) {
                ...
                this.lockedByOther = true
                this.lockedByName = this.orderList[j].lockedByUser
              }
              break
    ...
    searchClick () {
      ...
      } else if (this.lockedByOther) {
        var lockedInfo = this.Gbl.LangFun(this.WL.text116, this.Gbl.wayMapUserLang) + this.lockedByName + this.Gbl.LangFun(this.WL.text117, this.Gbl.wayMapUserLang)
        this.$emit('fInfoFun', ['errorInfo', lockedInfo])
      } else if (this.isCanSubmit) {
        var locationAddr = this.Gbl.wayMapLocationId + ''
        if ((this.orderList[this.selectedIndex].locationId + '') === locationAddr) {
          if (this.orderList[this.selectedIndex].transactionType === 'SO_WAVE' && this.orderList[this.selectedIndex].pickup) {
            this.router.push('/salesDetail?orderId=' + this.purchaseNum + '&orderType=' + this.orderType)
          } else {
            this.$emit('fInfoFun', ['errorInfo', this.Gbl.LangFun(this.WL.text128, this.Gbl.wayMapUserLang)])
          }
        } else {
