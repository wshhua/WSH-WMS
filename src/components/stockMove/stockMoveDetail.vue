  ...
  <div class='pageVM' id='pageVM' >
    ...
    <div class="content">
      ...
      <div class="item2">
        ...
        <div class="right">
          <input type="text" class="barCode" v-model="barCode" v-on:click.stop="barCodeClickFun" v-on:keyup.enter="barCodeEnter" ref="barcode"/>
      ...
      <div class="item9">
        <div class="left">{{this.Gbl.LangFun(this.WL.text84, this.Gbl.wayMapUserLang)}}:</div>
        <div class="right">
          <input type="text" class="batchNum" v-model="binNumber" ref="item9input" v-on:keyup.enter="batchNumEnter" v-on:click="inputBinNumberClick"/>
          <div class="binNameList" v-show="binNameListDir">
            ...
            v-bind:key="'index'+index">{{item.binName}}</div>
          </div>
        </div>
      </div>
      <div class="item3">
      ...
      <div class="item5">
        <div class="left">{{this.Gbl.LangFun(this.WL.text83, this.Gbl.wayMapUserLang)}}:</div>
      ...
      <div class="item7" v-bind:class="{active:item7Dir}">
        ...
        <input type="number" v-model="alreadyNum" class="productNum" ref="item7input" v-show="item7Dir"/>
...
export default {
  ...
  methods: {
    ...
    exitScanClick () {
      this.router.push('/stockMoveList?orderId=' + this.orderCode + '&orderType=' + this.orderType)
    },
    ...
    gainErrorRetrun (val1) {
      if (val1 === 'errorCode') {
        ...
        this.$refs.barcode.focus()
      } else if (val1 === 'gainTaskData') {
        window.location.reload()
      } else if (val1 === 'binNumber') {
        this.$refs.item9input.focus()
      }
    },
    gainOrderDetail () {
      ...
      this.$emit('loadingFun', true)
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTransationDetailList' + this.Gbl.commonVariaList + '&tranid=' + this.orderCode + '&transactionType=' + this.orderType
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
    ...
    itemClick (event) {
      this.binNumber = event.target.innerHTML
      this.binNameListDir = false
    },
    pageOtherClick () {
      this.barCodeListDir = false
    ...
    submintClick () {
      if (this.inTask) {
        if ((this.binNumber + '') === '') {
          this.$emit('fInfoFun', ['errorInfo', this.Gbl.LangFun(this.WL.text109, this.Gbl.wayMapUserLang), 'binNumber'])
        } else {
    ...
    upDataToPage (val1) {
      ...
      if (val1 < 0) {
        ...
        this.pdtDetail.binName = 'PUT ON BIN'
  ...
  watch: {
    alreadyNum () {
      if ((this.pdtDetail.waitNum - 0) <= 0 && (this.pdtDetail.pdtNum - 0) > 0) {
      ...
      } else if ((this.pdtDetail.pdtNum - 0) > 0) {
        if ((this.alreadyNum - 0) > (this.pdtDetail.waitNum - 0)) {
          this.$emit('fInfoFun', ['errorInfo', this.Gbl.LangFun(this.WL.text108, this.Gbl.wayMapUserLang), 'alreadyNum'])
          this.alreadyNum = this.pdtDetail.waitNum
...
.pageVM>.content>.item9 .binNameList {
  -webkit-box-sizing: border-box;
  -moz-box-sizing: border-box;
  box-sizing: border-box;
  width: 86%;
...
.pageVM>.content>.item9 .binNameItem {
  -webkit-box-sizing: border-box;
  -moz-box-sizing: border-box;
  box-sizing: border-box;
  width: 100%;
