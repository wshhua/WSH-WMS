<template>
  <div class='pageVM' id='pageVM' >
    ...
    <div class="content">
      ...
      <div class="noOrder" v-on:click="noOrderClick">{{this.Gbl.LangFun(this.WL.text85, this.Gbl.wayMapUserLang)}}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    gainAllotList () {
      ...
      // 获取清单数据
      var loginUrl1 = this.Gbl.commonUrl + '/transasctions/getTranidList' + this.Gbl.commonVariaList + '&transactionType=BIN_TRANSFER_TASK'
      this.axios({
        method: 'get',
        url: loginUrl1,
        data: '',
    ...
    noOrderClick () {
      this.router.push('/noOrderDetail')
    },
    searchClick () {
      this.isCanSubmitFun()
      if (this.purchaseNum === '') {
        ...
        this.router.push('/stockMoveDetail?orderId=' + this.purchaseNum + '&orderType=' + this.orderType)
      } else if (!this.isCanSubmit) {
