...
export default {
  ...
  data () {
    return {
      headerInfo: {
        goBackUrl: '/stockMoveDetail?orderId=' + this.Gbl.GainUrlValueFun('orderId') + '&orderType=' + this.Gbl.GainUrlValueFun('orderType'),
        homeDir: true,
        logoDir: false,
        homePopCtr: true,
        backPopCtr: false,
      ...
      this.axios({
        ...
        if ((response.status + '') === '200') {
          if ((response.data.status + '') === '200') {
            ...
            this.router.push('/successResult?downUrl=stockMoveSearch&downName=' + encodeURIComponent(downName))
          } else if ((response.data.errorCode + '') === '4000' || (response.data.errorCode + '') === '4001' || (response.data.errorCode + '') === '4002') {
            this.$emit('fInfoFun', ['errorInfo', response.data.msg, 'errorCode'])
          } else {
            console.log(response.data)
